function [ curv ] = czx_calCurvature_face(shape)
    ...
    n_triv = length(triv);
    p_direction = cell(n_triv,1);
    f_k1 = zeros(n_triv,1);
    f_k2 = zeros(n_triv,1);
    f_G = zeros(n_triv,1);
    f_H = zeros(n_triv,1);
    f_inner = zeros(n_triv,1);
    for i=1:n_triv
        fprintf('%d/%d\n',i,n_triv);
        %e1 = v(triv(i,1),:);
        %e2 = v(triv(i,2),:);
        ...
        %rot_mat = cal_rot_mat(normal); % rotate to (0,0,1)'s direction
        neighbor = find_face_nei_ring(triv,v,i,2);
        v_nei = v(neighbor,:);
        %v_nei = v_nei * rot_mat';
        [~,B,C,D,E,F] = fit_surface(v_nei);
        v0 = (v(triv(i,1),:)+v(triv(i,2),:)+v(triv(i,3),:))/3;
        %v0 = v0 * rot_mat';
        ...
        m = dxy/sqrt_eg_f2;
        n = dyy/sqrt_eg_f2;
%         weingarten_mat = [(f*m-g*l)/eg_f2, (f*l-e*m)/eg_f2; (f*n-g*m)/eg_f2, (f*m-e*n)/eg_f2];
%         [eig_V, eig_D] = eig(weingarten_mat);
        aa = n*f-m*g;
        bb = n*e-g*l;
        cc = m*e-f*l;
    ...
    curv = struct('direc', p_direction, 'k1', f_k1, 'k2', f_k2, 'G', f_G, 'H', f_H, 'inner', f_inner);
end


function [neighbor] = find_face_nei_ring(triv,v,i,max_depth)
    depth = 0;
    next_depth_num = 0;
    flag = zeros(1,length(v)); % 1 if added to queue; 0 otherwise.
    queue = [];
    q_front = 1; q_tail = 1;
    queue(q_tail) = triv(i,1); q_tail = q_tail + 1; next_depth_num = next_depth_num + 1; flag(triv(i,1)) = 1;
    queue(q_tail) = triv(i,2); q_tail = q_tail + 1; next_depth_num = next_depth_num + 1; flag(triv(i,2)) = 1;
    queue(q_tail) = triv(i,3); q_tail = q_tail + 1; next_depth_num = next_depth_num + 1; flag(triv(i,3)) = 1;
