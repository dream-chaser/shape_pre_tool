function [M] = calcLB_ACNN(shape, theta)
% The L-B operator matrix is computed by DiagS^-1*M.

% Calculate the weights matrix M
M = calcCotMatrixM1([shape.X, shape.Y, shape.Z], shape.TRIV, shape.curv, theta);
end


% ----------------------------------------------------------------------- %
function [M] = calcCotMatrixM1(Vertices, Tri, curv, theta)
...
a_dot_1 = zeros(size(Tri, 1),1);
a_dot_2 = zeros(size(Tri, 1),1);
a_dot_3 = zeros(size(Tri, 1),1);
alpha = 100;
for k = 1:size(Tri, 1)
    %D = diag([1/(1+alpha*abs(curv.k1(k))),1/(1+alpha*abs(curv.k2(k))),1]);
    D = diag([alpha,1,1]);
    U = [curv.k1_dir(k,:)',curv.k2_dir(k,:)',cross(curv.k1_dir(k,:),curv.k2_dir(k,:))'];
...
tmp1 = dot( v1,  v2, 2); cot1 = a_dot_1./sqrt(normVecSqr(v1).*normVecSqr(v2) - (tmp1).^2); clear tmp1;clear a_dot_1;
tmp2 = dot(-v1,  v3, 2); cot2 = a_dot_2./sqrt(normVecSqr(v1).*normVecSqr(v3) - (tmp2).^2); clear tmp2;clear a_dot_2;
tmp3 = dot(-v2, -v3, 2); cot3 = a_dot_3./sqrt(normVecSqr(v2).*normVecSqr(v3) - (tmp3).^2); clear tmp3;clear a_dot_3;
...
clear cot1;clear cot2;clear cot3;
